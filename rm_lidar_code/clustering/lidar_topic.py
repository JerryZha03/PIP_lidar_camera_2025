...
class Mid70PointCloudSubscriber(Node):
    def __init__(self):
        ...
        # 创建聚类中心发布者，使用 Float32MultiArray 消息类型
        ...
        # 创建滤波后点云发布者
        self.filtered_pcd_publisher = self.create_publisher(
            PointCloud2,
    ...
    def topic_callback(self, msg):
        try:
            ...
            voxel_size = 0.05  # 设置体素参数
            filtered_pcd = filtered_pcd.voxel_down_sample(voxel_size)  # 对名为data的点云进行下采样

            # DBSCAN 聚类
            eps = 0.28  # 聚类半径
            min_points = 40  # 有效聚类最小点数

            with o3d.utility.VerbosityContextManager(
                    o3d.utility.VerbosityLevel.Info) as cm:
                labels = np.array(filtered_pcd.cluster_dbscan(eps, min_points, print_progress=False))
            ...
            # 创建 Float32MultiArray 消息并发布所有聚类中心
            array_msg = Float32MultiArray()
            for center in centers:
                array_msg.data.extend([float(center[0]), float(center[1]), float(center[2])])
            self.cluster_center_publisher.publish(array_msg)
            ...
            if len(filtered_pcd.points) > 0:
                ros_msg = pc2.create_cloud_xyz32(msg.header, np.asarray(filtered_pcd.points))
                ros_msg.header.stamp = msg.header.stamp  # 更新时间戳
                self.filtered_pcd_publisher.publish(ros_msg)
            else:
                self.get_logger().warn("No points to publish after filtering")
